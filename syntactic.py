...
def p_tipo(p):
  ...
  if p[1] == "inteiro":
    ...
    filho1 = MyNode(name='TEXTO', type='TEXTO', parent=pai)
    filho_sym = MyNode(name=p[1], type=p[1].upper(), parent=filho1)
  p[1] = filho1
...
def p_operador_soma(p):
  ...
  """
  if p[1] == "+":
    soma = MyNode(name='SOMA', type='SOMA')
    soma_lexema = MyNode(name='+', type='SIMBOLO', parent=soma)
    p[0] = MyNode(name='operador_soma',
                  type='OPERADOR_SOMA', children=[soma])
  else:
    subtracao = MyNode(name='SUBTRACAO', type='SUBTRACAO')
    subtracao_lexema = MyNode(name='-', type='SIMBOLO', parent=subtracao)
    p[0] = MyNode(name='operador_soma',
                  type='OPERADOR_SOMA', children=[subtracao])
...
def p_caracter(p):
  ...
  pai = MyNode(name='caracter', type='CARACTER')
  p[0] = pai
  aux = MyNode(name='CARACTER', type='CARACTER', parent=pai)
...
def p_numero(p):
  ...
  if str(p[1]).find('.') == -1:
    aux = MyNode(name='NUMERO_INTEIRO', type='NUMERO_INTEIRO', parent=pai)
    aux_val = MyNode(name=p[1], type='VALOR', parent=aux)
    p[1] = aux
  elif str(p[1]).find('e') >= 0:
    aux = MyNode(name='NUMERO_CIENTIFICO',
                 type='NUMERO_CIENTIFICO', parent=pai)
    aux_val = MyNode(name=p[1], type='VALOR', parent=aux)
    p[1] = aux
  else:
    aux = MyNode(name='NUMERO_FLOAT',
                 type='NUMERO_FLOAT', parent=pai)
    aux_val = MyNode(name=p[1], type='VALOR', parent=aux)
    p[1] = aux
...
def main():
  ...
  if root and root.children != ():
    ...
    UniqueDotExporter(root).to_picture(argv[1] + "_unique.ast.png")
    UniqueDotExporter(root).to_dotfile(argv[1] + "_unique.ast.dot")
    # print(RenderTree(root, style=AsciiStyle()).by_attr())
    print("Graph was generated.\nOutput file: " + argv[1] + "_unique.ast.png")
